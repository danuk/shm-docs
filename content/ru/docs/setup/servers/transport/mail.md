...
{{< alert color="warning" title="Если видите ошибку AUTH:" >}}
Ошибка вида `failed AUTH: Command unknown: 'AUTH'` обычно означает, что авторизация началась до включения TLS. Для порта `587` используйте `starttls=1` (или не задавайте флаги и оставьте автоопределение по порту), для порта `465` — `ssl=1`.
{{< /alert >}}
...
to
: email адресата. Если не указан, автоматически заполняется из `user.email` — метода, который последовательно проверяет email из настроек пользователя, логина и профиля (подробнее: [Шаблоны → объект user]({{< ref "docs/setup/templates" >}}))
