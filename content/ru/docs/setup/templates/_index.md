...
| user.login | Логин пользователя
| user.login2 | Логин пользователя (дополнительный)
| user.email | Email пользователя. Последовательно проверяет: settings пользователя → login → login2 → профиль. Возвращает первый валидный адрес.
| user.balance | Баланс пользователя
...
| [user.make_autopayment( SUM )]({{< ref "/docs/setup/templates/notifications/forecast" >}}) | Выполняет автоплатеж на сумму `SUM`, если для пользователя доступен и разрешен автоплатеж
...
| [service.price_list()]({{< ref "service_price_list" >}}) | Возвращает прайс-лист услуг с расчетом скидок и бонусов
| [service.price_list_check_allow_to_order]({{< ref "service_price_list#проверка-доступности-услуги-для-заказа" >}}) | Проверка, доступна ли услуга для регистрации в текущем контексте пользователя
...
| Метод | Описание |
|:------|----------|
| bonus.id | Идентификатор бонуса
| bonus.date | Дата создания бонуса
| bonus.bonus | Кол-во бонусов
| bonus.comment | Комментарий
| bonus.amount | Кол-во бонусов (псевдоним для `user.get_bonus`)
...
| list.sort_by_key( foo = 'asc', bar = 'desc' ) | Виртуальный метод списка. Сортирует массив хешей/объектов по нескольким полям. Поддерживаемые направления: `asc`, `desc`.
| list.pluck( 'foo' ) | Виртуальный метод списка. Возвращает массив значений поля `foo` из каждого элемента списка.
