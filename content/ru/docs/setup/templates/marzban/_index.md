...
- Отредактируйте скрипт `marzban`:
  -- в переменную: `SUBSCRIPTION_DOMAIN` пропишите ваш домен, который указывает на Ваш сервер. Этот домен крайне важен для выпуска SSL сертификата.
  -- в переменную: `EMAIL_FOR_CERTIFICATE_ISSUE` пропишите Ваш реальный email. Он нужен для выпуска SSL сертификата для вашего домена.
