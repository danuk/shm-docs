...
Например, если выбрана группа серверов, транспорт которой определен как ["SSH"]({{< ref "/docs/setup/servers/transport/ssh" >}}), то можно настроить поле "Команда" (`cmd`),
которая должна быть выполнена на конечном сервере.
